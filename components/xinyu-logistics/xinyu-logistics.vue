	...
	<view class="page">
		...
		<view class="content bgf">
			<view>
				...
				<view class="flex list" :class="{one: index == 0 && delivery_status == 1}" v-for="(item, index) in wlInfo.list"
				 :key="index">
					<view class="time">
					...
					<view class="info flex1">
						<view class="title">{{index == 0 && delivery_status == 1 ? '该物流已被签收' : '配送中'}}</view>
...
<script>
	export default {
		...
		mounted(){
		  let _this=this;
		  console.log(this.wlInfo.delivery_status,9999);    
						if(this.wlInfo.delivery_status == '该物流已被签收'){
		...
		methods: {
			// changedelivery(){
			// 	console.log(this.wlInfo.delivery_status)
			// }
		}
	}
</script>
